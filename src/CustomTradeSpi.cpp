...
TThostFtdcOrderRefType	order_ref;	//报单引用 没用
time_t lOrderTime;
time_t lOrderOkTime;
...
{
	std::cout << "=====交易端口建立网络连接成功=====" << std::endl;
	// 开始登录
	reqUserLogin();
}
...
{
	std::cerr << "=====交易端口网络连接断开=====" << std::endl;
	std::cerr << "交易端口错误码： " << nReason << std::endl;
}

void CustomTradeSpi::OnHeartBeatWarning(int nTimeLapse)
{
	std::cerr << "=====交易端口网络心跳超时=====" << std::endl;
	std::cerr << "交易端口距上次连接时间： " << nTimeLapse << std::endl;
}
	...
	bool bIsLast)
{
	if (!isErrorRspInfo(pRspInfo))
	{
		std::cout << "=====交易端口投资者结算结果确认成功=====" << std::endl;
	...
	bool bIsLast)
{
	if (!isErrorRspInfo(pRspInfo))
	{
		std::cout << "=====交易端口查询投资者资金账户成功=====" << std::endl;
...
void CustomTradeSpi::OnRspQryInvestorPosition(
	...
	bool bIsLast)
{
	g_InvestorPosition = pInvestorPosition;

	if (!isErrorRspInfo(pRspInfo))
	{
		std::cout << "=====交易端口查询投资者持仓成功=====" << std::endl;
	...
	bool bIsLast)
{
	if (!isErrorRspInfo(pRspInfo))
	{
		...
		tradeOrderRefs[pInputOrder->OrderRef] = 0;	// 下单失败
	}
}
...
{
	// 成交回调
	//CallBackCloseOrder(true, int(pTrade->OrderRef));
	tradeOrderRefs[pTrade->OrderRef] = 1;	// 下单成功
	std::cout << "=====交易端口报单成功成交=====" << std::endl;
...
bool CustomTradeSpi::isErrorRspInfo(CThostFtdcRspInfoField *pRspInfo)
{
	...
	if (bResult)
		std::cerr << "交易端口返回错误--->>> ErrorID=" << pRspInfo->ErrorID << ", ErrorMsg=" << pRspInfo->ErrorMsg << std::endl;
	return bResult;
}
...
void CustomTradeSpi::reqUserLogin()
{
	...
	if (!rt)
		std::cout << ">>>>>>交易端口发送登录请求成功" << std::endl;
	else
		std::cerr << "--->>>交易端口发送登录请求失败" << std::endl;
}
...
void CustomTradeSpi::reqUserLogout()
{
	...
	if (!rt)
		std::cout << ">>>>>>交易端口发送登出请求成功" << std::endl;
	else
		std::cerr << "--->>>交易端口发送登出请求失败" << std::endl;
}
...
void CustomTradeSpi::reqSettlementInfoConfirm()
{
	...
	if (!rt)
		std::cout << ">>>>>>交易端口发送投资者结算结果确认请求成功" << std::endl;
	else
		std::cerr << "--->>>交易端口发送投资者结算结果确认请求失败" << std::endl;
}
...
void CustomTradeSpi::reqQueryInstrument()
{
	...
	if (!rt)
		std::cout << ">>>>>>交易端口发送合约查询请求成功" << std::endl;
	else
		std::cerr << "--->>>交易端口发送合约查询请求失败" << std::endl;
}
...
void CustomTradeSpi::reqQueryTradingAccount()
{
	...
	if (!rt)
		std::cout << ">>>>>>交易端口发送投资者资金账户查询请求成功" << std::endl;
	else
		std::cerr << "--->>>交易端口发送投资者资金账户查询请求失败" << std::endl;
}
...
void CustomTradeSpi::reqQueryInvestorPosition()
{
	...
	if (!rt)
		std::cout << ">>>>>>交易端口发送投资者持仓查询请求成功" << std::endl;
	else
		std::cerr << "--->>>交易端口发送投资者持仓查询请求失败" << std::endl;
}
...
void CustomTradeSpi::reqOrderInsert()
{
	...
	if (!rt)
		std::cout << ">>>>>>交易端口发送报单录入请求成功" << std::endl;
	else
		std::cerr << "--->>>交易端口发送报单录入请求失败" << std::endl;
}
...
void CustomTradeSpi::reqOrderAction(CThostFtdcOrderField *pOrder)
{
	...
	if (!rt)
		std::cout << ">>>>>>交易端口发送报单操作请求成功" << std::endl;
	else
		std::cerr << "--->>>交易端口发送报单操作请求失败" << std::endl;
...
int CustomTradeSpi::InsertOrder(char *InstrumentID, char dir, char offsetFlag, char priceType, double price, int num)
{
	CThostFtdcInputOrderField orderInsertReq;
	memset(&orderInsertReq, 0, sizeof(orderInsertReq));
	///经纪公司代码
	...
	///报单引用
	++(iOrderRef);
	_snprintf_s(orderInsertReq.OrderRef, sizeof(TThostFtdcOrderRefType), sizeof(TThostFtdcOrderRefType) - 1, "%012d", iOrderRef);
	//strcpy(orderInsertReq.OrderRef, order_ref);

	///报单价格条件: 限价
	orderInsertReq.OrderPriceType = priceType; //THOST_FTDC_OPT_LimitPrice;
	///买卖方向: 
	orderInsertReq.Direction = dir;
	///组合开平标志: 开仓
	...
	orderInsertReq.LimitPrice = price;
	///数量：1
	orderInsertReq.VolumeTotalOriginal = num;
	///有效期类型: 当日有效
	orderInsertReq.TimeCondition = THOST_FTDC_TC_GFD;
	///成交量类型: 任何数量
	orderInsertReq.VolumeCondition = THOST_FTDC_VC_AV;
	...
	static int requestID = 0; // 请求编号
	int rt = g_pTradeUserApi->ReqOrderInsert(&orderInsertReq, ++requestID);
	char * msg = rt == 0 ? "success:下单成功" : "failer:下单失败";
...
// 撤销订单
int CustomTradeSpi::DeleteOrder(char *InstrumentID, int orderRef)
{
	//错误返回-1是和正常冲突的吗？
	if (!InstrumentID)
	{
	...
	strcpy_s(ReqDel.BrokerID, sizeof(TThostFtdcBrokerIDType), gBrokerID);
	strcpy_s(ReqDel.InvestorID, sizeof(TThostFtdcInvestorIDType), gInvesterID);
	...
	ReqDel.FrontID = trade_front_id;
	ReqDel.SessionID = session_id;
	ReqDel.ActionFlag = THOST_FTDC_AF_Delete;
	int iResult = g_pTradeUserApi->ReqOrderAction(&ReqDel, ++(iRequestID));

	if (iResult != 0)
		std::cout << "Failer: 撤单 : " << ((iResult == 0) ? "成功" : "失败(") << iResult << ")" << std::endl;
	else
		std::cout << "Scuess: 撤单 : 成功" << std::endl;
